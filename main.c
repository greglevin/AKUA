...
static struct net_buf snd_buf;
static u8_t rxmsg[BSM_MAX_MSG_LEN];
static struct net_buf rcv_buf;
...
void main(void)
{
    ...
    struct net_buf *buf = &rcv_buf;
    k_tid_t tx_id;
    int err;
    
        rcv_buf.data = &rxmsg[0];
...
*/
	...
	tx_id = k_thread_create(&bt_tx_thread_data, bt_tx_thread_stack,
				...
				bt_tx_thread, NULL, NULL, NULL, 7,
				0, K_NO_WAIT);

	/* Implement notification. At the moment there is no suitable way
	 * of starting delayed work so we do it here
	 */
       printk("Sending thread started\n");
	...
	while (1) {
		...
		cts_notify();
                printk("Receiving thread\n");
                memcpy(buf,net_buf_get(&rx_queue, K_FOREVER),sizeof(struct net_buf));
